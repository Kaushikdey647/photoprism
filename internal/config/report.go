...
func (c *Config) Report() (rows [][]string, cols []string) {
	...
	rows = append(rows, [][]string{
		...
		{"disable-cull", fmt.Sprintf("%t", !c.CullEnabled())},
		{"cull-schedule", c.CullSchedule()},
		{"cull-filter", c.CullFilter()},
		{"cull-window", fmt.Sprintf("%d", int(c.CullWindow().Seconds()))},
		{"cull-diff", fmt.Sprintf("%d", c.CullDiff())},
		{"cull-same-camera", fmt.Sprintf("%t", c.CullSameCamera())},
		{"cull-min", fmt.Sprintf("%d", c.CullMin())},
		{"cull-auto-archive", fmt.Sprintf("%t", c.CullAutoArchive())},
		{"cull-skip-favorites", fmt.Sprintf("%t", c.CullSkipFavorites())},
